...
CREATE TABLE IF NOT EXISTS `veganerLand`.`ORDERITEMS` (
  ...
  INDEX `fk_ORDERITEMS_PRODUCTS_idx` (`PRODID` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `veganerLand`.`ORDERS` (
  ...
  `ITEMID` INT NOT NULL,
  `CUSTID` INT NOT NULL,
  `ORDERDATE` DATETIME NOT NULL DEFAULT CURRENT_DATETIME,
  `SHIPDATE` DATETIME NULL,
  PRIMARY KEY (`ORDERID`),
  UNIQUE INDEX `ORDERID_UNIQUE` (`ORDERID` ASC) VISIBLE,
  INDEX `fk_ORDERS_ORDERITEMS1_idx` (`ITEMID` ASC) VISIBLE,
  INDEX `fk_ORDERS_CUSTOMER1_idx` (`CUSTID` ASC) VISIBLE,
